...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int n = sc.nextInt();
        int numOfFriends = 3;
        int[][] input = new int[n][numOfFriends];

        for (int r = 0; r < n; ++r) {
            for (int c = 0; c < numOfFriends; ++c)
        ...
        for (int r = 0; r < n; ++r) {
            int count = 0;
            for (int c = 0; c < numOfFriends; ++c) {
                //If the cell equals to 1, it means that a team member is sure about the solution
                if (input[r][c] == 1)
                    ++count;
            }
            //They will implement the problem if 2 members from team are sure about the solution
            if (count >= 2)
                ++result;
        }
